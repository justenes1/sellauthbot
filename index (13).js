...
module.exports = {
    ...
    // Buy command (with random order ID)
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (paymentMethod.toLowerCase() !== 'ltc') {
            const pm = db.getPaymentMethodByName(paymentMethod);
            if (!pm) {
...
module.exports = {
    ...
    // Add Product command (with random product ID)
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const result = db.addProduct(name, description, price, stock, imageUrl);
        ...
        console.log(\`📦 Product added: \${result.productId} - \${name}\`);
        await interaction.reply({ embeds: [embed], ephemeral: true });
    }
};`;
...
module.exports = {
    ...
    console.log('✅ Created 16 command files + payment checker');
}
